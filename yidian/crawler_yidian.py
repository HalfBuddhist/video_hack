...
payload = {'clientInfo%5BuserInfo%5D%5Bcountry%5D': 'CN', 'clientInfo%5BdeviceInfo%5D%5BscreenWidth%5D': '750',
           ...
           'clientInfo%5BuserInfo%5D%5Bregion%5D': '%E4%B8%AD%E5%9B%BD', 'clientInfo%5BdeviceInfo%5D%5Bdevice%5D': '',
           'platform': '0', 'version': '020600', 'net': 'wifi',
           'clientInfo%5BuserInfo%5D%5Bifa%5D': '5ef9a039be1b5ae299140646e83ce2f5',
           'clientInfo%5BdeviceInfo%5D%5BiosVersion%5D': '11.3', 'clientInfo%5BdeviceInfo%5D%5BscreenResolution%5D': '',
           'clientInfo%5BdeviceInfo%5D%5BscreenHeight%5D': '1334',
           'clientInfo%5BuserInfo%5D%5BappVersion%5D': '4.6.2.1', 'clientInfo%5BuserInfo%5D%5Blanguage%5D': 'zh-Hans',
           'clientInfo%5BdeviceInfo%5D%5BscreenDensity%5D': '', 'appid': 'pro', 'clientInfo%5BuserInfo%5D%5Bmac%5D': '',
...
while True:
    ...
    # print type(url_video_cat_root_path)
    ...
    print(url_crawl)
    res = requests.post(url_crawl, data=payload, cookies=jar)
    print(res.status_code)
    # print(res.text)  # in utf-8

    res_json = json.loads(res.text, encoding="utf-8")
    if res_json['status'] == u'failed' and res_json['code'] == 0 and res_json['reason'] == 'BLENDER_RESULT is null':
        # crawl to the end.
        print("Crawling failed for: %s." % res_json['reason'])
        print("Crawling Finished, found %s videos." % cnt_video)
        break
    elif res_json['status'] == u'failed' and res_json['code'] == 250 and res_json['reason'] == 'user access forbidden':
        # forbidden by ip
        print("Crawling failed for: %s." % res_json['reason'])
        print("Sleep %s minutes to restart." % run_param.sleep_base_when_forbidden)
        sys.stdout.flush()
        ...
        print("Found %s videos." % (len(res_json["result"])))
        ...
        for item in result:
            ...
            if 'title' in item:
                ...
                url_video = item['video_url'].replace('\\', '')
        ...
        print(res_json)
        print("Crawling failed for: %s." % res_json['reason'])
        print("Crawling failed, found %s videos." % cnt_video)
        break
